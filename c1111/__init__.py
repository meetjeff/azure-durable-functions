...
def main(ser:str) -> str:
    ...
    logging.info('1111 Start Search')
    ...
    for p in range(1,rp):
        try:        
            title = requests.get(f"https://www.1111.com.tw/search/job?ks={sp}&page={p}")
    ...
    logging.info(f"已新增 {jdata['search']} 職缺查詢至 1111 ({jdata['page']} 頁) !")

    return f"已新增 {jdata['search']} 職缺查詢至 1111 ({jdata['page']} 頁) !"
